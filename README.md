...
Put the screenshot at the path you referenced under `public/images/projects/`, then run:

```bash
npm run build-images
```

This generates the responsive WebP variants the project card actually serves (see
"Images and responsive variants" below). If the file isn't there yet at all, the card
shows a neutral placeholder instead of a broken image; the build never fails because an
asset is missing.
...
`verificationUrl` or `certificateFile`, not both: if both are present, the verification
link is shown. An entry with neither renders as a finished card with no link, which is
intentional for the awards that have nothing to link to.
...
## Images, responsive variants and missing assets

Every image on the site (portrait, project screenshots) goes through
`src/components/ImageWithFallback.tsx`, which renders a `<picture>` with WebP `srcset`
variants and falls back in two stages: if the optimized WebP is missing, it drops back
to the original raster; if that's missing too, it swaps in a neutral bordered
placeholder instead of a broken image icon. CV and certificate links are plain `<a>`
tags: they render normally whether or not the target file exists yet.

The WebP variants (and the Open Graph social card) are generated by a script, not by
Next itself, because this site uses `output: "export"` with `images.unoptimized: true`
(GitHub Pages has no image server to resize on request). Run it after adding or
replacing any image under `public/images/`:

```bash
npm run build-images
```

This reads `public/images/portrait.jpeg` and everything under
`public/images/projects/`, writes `<name>-<width>.webp` variants alongside each source
image, and regenerates `public/images/og-image.jpg` (the 1200x630 social-card image used
in Open Graph / Twitter card metadata, composed from the palette, the portrait and the
name/headline text). Commit the generated files alongside the source image.
